...
extern void print_eigenvalues( const char* desc, MKL_INT n, double* wr, double* wi );
extern void print_eigenvectors( const char* desc, MKL_INT n, double* wi, double* v,
      MKL_INT ldv );
...
int main() {
        ...
        info = LAPACKE_dgeev( LAPACK_ROW_MAJOR, 'N', 'N', n, a, lda, wr, wi,
                        NULL, ldvl, NULL, ldvr );
...
void print_eigenvalues( const char* desc, MKL_INT n, double* wr, double* wi ) {
        MKL_INT j;
        printf( "\n %s\n", desc );
   for( j = 0; j < n; j++ ) {
      if( wi[j] == (double)0.0 ) {
...
void print_eigenvectors( const char* desc, MKL_INT n, double* wi, double* v, MKL_INT ldv ) {
        MKL_INT i, j;
        printf( "\n %s\n", desc );
   for( i = 0; i < n; i++ ) {
      j = 0;
